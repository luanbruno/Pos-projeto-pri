...
public class LubrificanteController {
	...
	public String incluir(Model model, Lubrificante lubrificante, @SessionAttribute("user") Usuario usuario) {
		...
		model.addAttribute("mensagem", "O lubrificante para " + lubrificante.getMotor() + " foi cadastrado com sucesso!!!");
	...
	public String excluir(Model model, @PathVariable Integer id, @SessionAttribute("user") Usuario usuario) {
		...
		try {
			...
			model.addAttribute("mensagem", "O lubrificante de "+  lubrificante.getMotor() +" foi removida com sucesso!!!");		
		} catch (Exception e) {
			model.addAttribute("mensagem", "Impossível remover o lubrificante para "+lubrificante.getMotor()+"!!! Ela está associada a uma venda.");
		}	
